...
def tetris(screen, data: PlayerData):
    try:
        fps = 25
        ...
        def pauseScreen():
            pause = pg.Surface((800, 600), pg.SRCALPHA)
            ...
            while True:
                ...
                if time.time() - last_fall > fall_speed:
                    if not checkPos(cup, fallingFig, adjY=1):
                        ...
                        rise = clearCompleted(cup)
                        points += rise
                        data.add_money(rise * 10, 'elephant')
                        data.save()
                        level, fall_speed = calcSpeed(points)
                        fallingFig = None
                    else:
        ...
        def stopGame():
            # намеренный вызов ошибки для переключения на следующий экран
            raise SyntaxError
        ...
        def drawTitle():
            ...
            titleRect.topleft = (window_w - 525, 30)
            display_surf.blit(titleSurf, titleRect)
